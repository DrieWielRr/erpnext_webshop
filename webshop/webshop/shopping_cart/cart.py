...
def update_cart(item_code, qty, additional_notes=None, with_items=False, configuration=None, is_add_to_cart=False):
	...
	if valid_configuration:
		...
		log(f"party name={quotation.get('party_name')}")
		log(f"party type={quotation.get('quotation_to')}")


		empty_card = False    
		if qty == 0:
			log(f"[ITEM_REMOVAL] qty: {qty}, total quotation items: {len(quotation.items)}")
			quotation_items = [
			...
			log(f"[ITEM_REMOVAL] quotation item qty after removal: {len(quotation_items)}")
			if quotation_items:
...
@frappe.whitelist()
def update_cart_ORIGINAL(item_code, qty, additional_notes=None, with_items=False):
	quotation = _get_cart_quotation()
